...
IMAGES_TO_DELETE=$(aws ecr list-images --region ap-southeast-2 --repository-name dylan-test-app --filter tagStatus=UNTAGGED --query imageIds[*] --output json)
aws ecr batch-delete-image --region ap-southeast-2 --repository-name dylan-test-app --image-ids "$IMAGES_TO_DELETE" || true